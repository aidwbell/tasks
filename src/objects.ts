...
export function isCorrect(question: Question, answer: string): boolean {
    ...
    const trimmedQuestion = question.expected.trim().toLowerCase();

    return trimmedAnswer === trimmedQuestion;
}
...
export function isValid(question: Question, answer: string): boolean {
    ...
    if (question.type === "multiple_choice_question" && question.options) {
        //const options: string[] = ["Option A", "Option B", "Option C"];
...
export function toMarkdown(question: Question): string {
    ...
    return markdown.trim(); // Trim any trailing newline characters
}
...
export function duplicateQuestion(id: number, oldQuestion: Question): Question {
    const duplicatedQuestion: Question = {
        id: id, // Use the provided ID for the new question
        name: `Copy of ${oldQuestion.name}`, // Modify the name as specified
...
export function mergeQuestion(
    ...
    const mergedQuestion: Question = {
        id: id, // Use the provided ID for the new question
        name: name, // Use the provided name for the new question
        body: contentQuestion.body,
        type: contentQuestion.type,
        options: contentQuestion.options,
        expected: contentQuestion.expected,
        points: points, // Use the points from the second question
        published: false // Set the published field to false
    };
